...
Argo Rollouts in {gitops-title} support various traffic management mechanisms such as xref:../extend/manage-argo-rollouts-lifecycle-and-traffic.adoc#routing-traffic-by-using-argo-rollouts[OpenShift Routes] and Istio-based OpenShift Service Mesh. You can use OpenShift Service Mesh to split traffic between two application versions:
