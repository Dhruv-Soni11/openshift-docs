...
* There is currently a known issue that causes decoupling application synchronization using impersonation to fail due to insufficient permissions in the `openshift-gitops-openshift-gitops-argocd-application-controller` `ClusterRole`. This `ClusterRole` lacks the necessary permissions required for impersonation, leading to a failure during application synchronization. When encountering this issue, you may see an error message similar to the following:
+
.Example
[source,YAML]
----
...
+
* Manually edit the `openshift-gitops-openshift-gitops-argocd-application-controller` ClusterRole to include the necessary permissions for impersonation.
* Add `ClusteRole` and `ClusterRolebBnding` for providing the required permission after enabling impersonation.
...
rules:
  - verbs:
    ...
    resources:
      - serviceaccounts
----
** Add a new `ClusterRole` to Argo CD Application Controller service account using a new `ClusterRoleBinding` role.
+
.Example
[source,YAML]
----
